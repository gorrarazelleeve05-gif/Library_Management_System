# LOCATION: backend/books/views.py
...
class MeView(APIView):
    """GET current user info. PATCH updates name/email + member profile fields."""
    ...
    def patch(self, request):
        ...
        user = request.user
        if 'first_name' in d: user.first_name = d['first_name']
        if 'last_name'  in d: user.last_name  = d['last_name']
        if 'email'      in d: user.email       = d['email']
        user.save()

        # Update Member table profile fields
        if not user.is_staff:
            try:
                member = user.member_profile
                if 'member_type' in d: member.member_type = d['member_type']
                if 'bio'         in d: member.bio         = d['bio']
                if 'photo_b64'   in d: member.photo_b64   = d['photo_b64']
...
class BookListCreateView(APIView):
    def get_permissions(self):
        if self.request.method == 'GET': return [IsAuthenticated()]
        return [IsAdmin()]
    ...
    def get(self, request):
        ...
        if search:    qs = qs.filter(Q(title__icontains=search)|Q(author__icontains=search)|Q(isbn__icontains=search))
        if genre:     qs = qs.filter(genre=genre)
        if available == 'true': qs = qs.filter(available_copies__gt=0)
...
class BookDetailView(APIView):
    ...
    def get_object(self, pk):
        try: return Book.objects.get(pk=pk)
        except Book.DoesNotExist: return None

    def get(self, request, pk):
        book = self.get_object(pk)
        if not book: return Response({'error': 'Book not found.'}, status=404)
    ...
    def delete(self, request, pk):
        ...
        if active: return Response({'error': f'Cannot delete — {active} active borrow(s) exist.'}, status=400)
        book.delete()
...
class BookDetailWithHistoryView(APIView):
    ...
    def get(self, request, pk):
        ...
        book.borrow_records.filter(status='borrowed', due_date__lt=date.today()).update(status='overdue')
        ...
        if not request.user.is_staff:
            try:
                ub = book.borrow_records.filter(member__user=request.user, status__in=['borrowed','overdue','pending']).first()
                if ub: user_borrow = BorrowRecordSerializer(ub).data
            except: pass
        return Response({'book': BookSerializer(book).data, 'records': BorrowRecordSerializer(records, many=True).data, 'user_borrow': user_borrow})
...
class MemberListCreateView(APIView):
    ...
    def get(self, request):
        ...
        search = request.query_params.get('search', '')
        if search:
            qs = qs.filter(Q(user__first_name__icontains=search)|Q(user__last_name__icontains=search)|Q(user__email__icontains=search)|Q(user__username__icontains=search))
...
class MemberDetailView(APIView):
    ...
    def get_object(self, pk):
        try: return Member.objects.select_related('user').get(pk=pk)
        except Member.DoesNotExist: return None

    def get(self, request, pk):
        m = self.get_object(pk)
        if not m: return Response({'error': 'Member not found.'}, status=404)
...
class BorrowRecordListCreateView(APIView):
    ...
    def get(self, request):
        ...
        if member_id and request.user.is_staff: qs = qs.filter(member_id=member_id)
...
class BorrowRecordDetailView(APIView):
    ...
    def get_object(self, pk, user):
        try:
            ...
            if not user.is_staff and record.member.user != user: return None
            return record
        except BorrowRecord.DoesNotExist: return None

    def get(self, request, pk):
        record = self.get_object(pk, request.user)
        if not record: return Response({'error': 'Not found.'}, status=404)
        return Response(BorrowRecordSerializer(record).data)

    def delete(self, request, pk):
        if not request.user.is_staff: return Response({'error': 'Only admins can delete.'}, status=403)
        record = self.get_object(pk, request.user)
        if not record: return Response({'error': 'Not found.'}, status=404)
        if record.status in ['borrowed','overdue']: return Response({'error': 'Return the book first.'}, status=400)
        record.delete()
...
class ApproveBorrowView(APIView):
    ...
    def post(self, request, pk):
        try: record = BorrowRecord.objects.select_related('book').get(pk=pk)
        except BorrowRecord.DoesNotExist: return Response({'error': 'Record not found.'}, status=404)
        if record.status != 'pending': return Response({'error': f'Cannot approve — status is "{record.status}".'}, status=400)
        if not record.book.is_available: return Response({'error': 'Book is no longer available.'}, status=400)
        s = ApproveBorrowSerializer(data=request.data)
        if not s.is_valid(): return Response(s.errors, status=400)
        record.status = 'borrowed'
        if s.validated_data.get('due_date'): record.due_date = s.validated_data['due_date']
        if s.validated_data.get('admin_notes'): record.admin_notes = s.validated_data['admin_notes']
        record.book.available_copies -= 1
        record.book.save()
        record.save()
        return Response(BorrowRecordSerializer(record).data)
...
class RejectBorrowView(APIView):
    permission_classes = [IsAdmin]
    def post(self, request, pk):
        try: record = BorrowRecord.objects.get(pk=pk)
        except BorrowRecord.DoesNotExist: return Response({'error': 'Record not found.'}, status=404)
        if record.status != 'pending': return Response({'error': f'Cannot reject — status is "{record.status}".'}, status=400)
...
class ReturnBookView(APIView):
    ...
    def post(self, request, pk):
        ...
        if record.status not in ['borrowed','overdue']: return Response({'error': f'Cannot return — status is "{record.status}".'}, status=400)
        s = ReturnBookSerializer(data=request.data)
        if not s.is_valid(): return Response(s.errors, status=400)
        record.return_date = date.today()
        record.status = 'returned'
        if s.validated_data.get('notes'): record.notes = s.validated_data['notes']
        record.save()
        record.book.available_copies += 1
        record.book.save()
        return Response(BorrowRecordSerializer(record).data)
